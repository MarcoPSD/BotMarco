...
async def photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        "Grande! ora mandami la tua posizione!, così possiamo metterti in contatto con il server più vicino.\n\nOppure scrivi /skip per settarla più avanti(potrebbe causare rallentamenti)."
    )
...
async def skip_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        "Hai paura per caso? Ora, mandami la tua posizione,\n\nOppure scrivi /skip per settarla in seguito."
    )
...
async def bio(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("Figo, ora che ci conosciamo meglio, puoi farmi delle domande sul mio creatore MARCO BRAGA.\n\n Seleziona la domanda disponibile!",
    reply_markup=ReplyKeyboardMarkup(
...
async def question1(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        ...
        "Ricordo che mi raccontava che appena un anno fa, mentre guardava una gara di BMX ad H-FARM, disse verso un suo amico: “io un giorno verrò a studiare in sto posto fighissimo”,  e alla fine c’è riuscito.\n\n"
...
async def question4(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Starts the conversation and asks the user about their gender."""
    reply_keyboard = [["Voglio sapere come sei stato Scriptato"]]
    user = update.message.from_user
...
async def end(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        ...
        "Un'ultima cosa...abbiamo accesso a tutti i dati che ci hai mandato ;)\n\n"
        "Ci vediamo presto!",
        reply_markup=ReplyKeyboardRemove()
    )
...
def main() -> None:
    ...
    application = Application.builder().token("6139608206:AAEG5nqg6ot91xYaQH_no4DWLd-QhDnTNBw").build()
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            MYSCRIPT: [MessageHandler(filters.Text("^((Voglio sapere come sei stato Scriptato)$"), myscript)]
